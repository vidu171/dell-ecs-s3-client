...
func New(cfg aws.Config, clientInfo metadata.ClientInfo, handlers Handlers,
	...
	var err error
	httpReq.URL, err = url.Parse(clientInfo.Endpoint + getTranslatedHTTPPath(operation.HTTPPath, params))
	fmt.Println("From request", httpReq.URL)
	if err != nil {
		httpReq.URL = &url.URL{}
...
func getTranslatedHTTPPath(HTTPPath string, params interface{}) string {
	val := reflect.ValueOf(params).Elem()
	translatedHttpPath := HTTPPath
	if val.FieldByName("Bucket").IsValid() {
		bucket := val.FieldByName("Bucket").Interface().(*string)
		translatedHttpPath = strings.Replace(translatedHttpPath, "{Bucket}", *bucket, 1)

	}
	if val.FieldByName("Key").IsValid() {
		key := val.FieldByName("Key").Interface().(*string)
		translatedHttpPath = strings.Replace(translatedHttpPath, "{Key+}", *key, 1)

	}
	return translatedHttpPath
}
